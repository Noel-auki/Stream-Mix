...
buffer_size = 10  # Adjust this size based on your needs
...
while True:
    ...
    if audio_packet and first_video_pts is not None:
        ...
        # Add to audio buffer
        audio_buffer.append(audio_packet)
    ...
    if video_packet:
        ...
        # Add to video buffer
        video_buffer.append(video_packet)

    # Process the buffer if both buffers are filled
    if len(video_buffer) == buffer_size and len(audio_buffer) == buffer_size:
        while video_buffer and audio_buffer:
            output_format.mux(video_buffer.popleft())
            output_format.mux(audio_buffer.popleft())
